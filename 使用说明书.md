...
**立即修复：运行 [ONE_CLICK_FIX.sql](ONE_CLICK_FIX.sql)**

如果在admin面板保存失败，显示 "Failed to save protocol: Failed to create protocol: new row violates row-level security policy"：

1. 打开 Supabase Dashboard → SQL Editor
2. 复制粘贴 `ONE_CLICK_FIX.sql` 的全部内容
3. 点击 Run 运行
4. 刷新浏览器
5. 完成！

详细步骤见 → [QUICK_FIX_GUIDE.md](QUICK_FIX_GUIDE.md)
...
│ INFRASTRUCTURE                           │
│                                          │
│ Decentralized GPU cloud computing...    │
│                                          │
│ $ Raised: $25.0M      📅 Stage: Series A │
│ $ Expected: $30       📅 Listed: 3       │
│ ──────────────────────────────────────── │
│ What to do:                              │
│ [Daily Check-in] [Staking] [Social...]   │
│ ──────────────────────────────────────── │
│ Lead Investors:                          │
│ [Framework Ventures] [Hashkey Capital]   │
│ ──────────────────────────────────────── │
│ Chains:                                  │
...
**卡片字段说明**：
- **Raised**: 融资金额（横向显示左侧）
- **Stage**: 融资阶段（横向显示右侧 - Seed/Series A/B/Pre-TGE/TGE）
- **Expected**: 预期参与成本（横向显示左侧）
- **Listed**: 项目上线天数（横向显示右侧）
...
**Q: Admin保存失败，显示"new row violates row-level security policy"？** ⭐⭐⭐

A: **这是RLS策略问题！立即修复：**

1. 打开 Supabase Dashboard → SQL Editor
2. 运行 [ONE_CLICK_FIX.sql](ONE_CLICK_FIX.sql) 文件的所有内容
3. 刷新浏览器
4. 完成！
